...
class PTBModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    bin_b = tf.sign(softmax_b, name="bin_b")
...
class MediumConfig(object):
  ...
  hidden_size = 300
  max_epoch = 5
  max_max_epoch = 16 #25 on saved model, about same as 16
  keep_prob = 0.8
  lr_decay = 0.8
  batch_size = 32
  vocab_size = 254000
...
def run_epoch(session, model, data, is_train=False, verbose=False):
  ...
  for step, (x, y) in enumerate(wiki_gen_reader.wiki_iterator(data, model.batch_size, model.num_steps)):
    if is_train:
      fetches = [model.cost, model.final_state, model.train_op, model.clip_op1, model.clip_op2]
    else:
...
def run_epoch_log(session, model, data, is_train=False, verbose=False):
  ...
  #print(len(list(enumerate(wiki_gen_reader.wiki_iterator(data, model.batch_size, model.num_steps))))) #
  for step, (x, y) in enumerate(wiki_gen_reader.wiki_iterator(data, model.batch_size, model.num_steps)):
    if is_train:
      fetches = [model.cost, model.final_state, model.train_op, model.clip_op1, model.clip_op2]
    else:
